...
def _process_question(question: str):
    """Internal helper to process a question and return a JSON-serializable result."""
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"error": "langchain-huggingface not installed. Run: pip install langchain-huggingface"},
        )
...
async def ask_question(question: str = Form(...)):
    try:
        return _process_question(question)
...
async def ask_question_get(question: str):
    """Convenience GET endpoint for browser testing: use ?question=..."""
    try:
        return _process_question(question)
    except Exception as e:
        logger.exception("Error processing question (GET)")
        return JSONResponse(status_code=500, content={"error": str(e)})